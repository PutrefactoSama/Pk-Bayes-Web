export async function onRequestPost(context) {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      // Función auxiliar segura para enviar vía Resend con fallback si el dominio no está verificado
      async function sendSafeEmail(toEmail, subject, htmlContent) {
            ...
            if (toEmail !== adminEmail) {
              ...
              await fetch("https://api.resend.com/emails", {
                ...
                body: JSON.stringify({
                  from: fromAddress,
                  to: adminEmail,
                  subject: `[Copia para Cliente: ${toEmail}] ${subject}`,
                  html: `
                    <div style="background:#fffbeb;border:1px solid #fef08a;padding:12px;border-radius:8px;margin-bottom:16px;font-size:13px;color:#854d0e">
                      ℹ️ <strong>Modo Prueba Resend:</strong> Este correo fue generado para <code>${toEmail}</code>. (Para enviarlo a correos externos directamente, agrega tu dominio en <a href="https://resend.com/domains">resend.com/domains</a>).
                    </div>
                    ${htmlContent}
                  `,
                }),
      ...
      // 1. Correo 1 al Cliente: Confirmación de Pago
      const correo1Html = `
      ...
      await sendSafeEmail(email, "[PK-Bayes] ¡Pago exitoso! Tu cuenta está siendo aprobada ⏳", correo1Html);

      // 2. Correo 2 al Cliente: Credenciales con clave temporal
      const correo2Html = `
        <div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;color:#1e293b;line-height:1.6">
          <div style="background:#0a192f;padding:26px 30px;color:#ffffff">
            <h1 style="margin:0;font-size:22px;letter-spacing:-0.5px">PK-Bayes</h1>
      ...
      await sendSafeEmail(email, "[PK-Bayes] Tu entorno clínico está listo — Credenciales de acceso", correo2Html);

      // 3. Alerta Interna para Ti (Pablo)
      const adminAlertHtml = `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e2e8f0;border-radius:12px;color:#1e293b">
          ...
          <p style="font-size:15px">Se ha procesado el pago y despachado la clave temporal al cliente.</p>
          ...
          <div style="background:#f0fdf4;border:1px solid #bbf7d0;padding:16px;border-radius:8px;margin-top:24px">
            <h4 style="margin:0 0 8px;color:#15803d">✅ Estado:</h4>
            <p style="margin:0;font-size:13px;color:#166534;line-height:1.6">
              • Correo 1: Confirmación de pago enviado.<br>
              • Correo 2: Credenciales con clave temporal <code>${tempPassword}</code> enviado.<br>
            </p>
          </div>
      ...
      await sendSafeEmail(adminEmail, `🟢 NUEVA VENTA - PK-Bayes: ${name}`, adminAlertHtml);
